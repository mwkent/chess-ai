...
class Board(chess.Board, object):
    ...
    def get_position_string(self) -> str:
        """Gets the FEN without the halfmove clock or fullmove number.
        """
        # Remove halfmove clock and fullmove number
        pattern = "\s\d+\s\d+$"
        position_string = re.sub(pattern, '', self.fen())
    ...
    def get_phase(self, color):
        if color not in self._phase:
            ...
            scaled_piece_value_total = min(min_opening_total,
                                           max(piece_value_total, max_endgame_total)) - max_endgame_total
    ...
    def get_first_attackers_and_defenders(self, square: chess.Square, defend_color: chess.Color = None) \
        ...
        attackers = [attacker for attacker in self.attackers(not defend_color, square) if
                     chess_util.can_piece_capture(self, attacker, square)]
        defenders = [attacker for attacker in self.attackers(defend_color, square) if
                     chess_util.can_piece_capture(self, attacker, square)]
    ...
    def is_attacker_soft_pinning(self, attacker: chess.Square, pinned_piece: chess.Square):
        ...
        for ray_square in chess.SquareSet.ray(attacker, pinned_piece):
            ...
            elif self.piece_type_at(ray_square) is not None and self.color_at(ray_square) == self.color_at(
                    pinned_piece) and \
    ...
    def has_defender(self, piece: chess.Square):
        defend_color = self.color_at(piece)
        return any(
            chess_util.can_piece_capture(self, defender, piece) for defender in self.attackers(defend_color, piece))
    ...
    def get_soft_second_defenders(self, square: chess.Square, first_attackers,
        ...
        if defend_color is None:
            defend_color = self.color_at(square)
        first_attackers_and_defenders = first_attackers + first_defenders
        second_defenders = [defender for defender in
                            self.get_battery_attackers(square, defend_color, first_attackers_and_defenders)
    ...
    def is_stronger_piece_attacked_by(self, attacking_piece: chess.Square,
        ...
        """Is `attacked_piece` of a higher value and different color than `attacking_piece`?
        ...
        attacked_piece_type = self.piece_type_at(attacked_piece)
        return self.piece_at(attacked_piece) is not None and \
            attacked_piece_type != chess.KING and \
            self.color_at(attacked_piece) != piece_color and \
            chess_util.can_piece_capture(self, attacking_piece, attacked_piece) and \
            piece_value < PIECE_TYPES_TO_ROUGH_VALUES[attacked_piece_type]
    ...
    def is_attacked_by_weaker_piece(self, piece: chess.Square) -> bool:
        """Is `piece` attacked by a weaker piece
        """
        piece_color = self.color_at(piece)
        return any(attacker for attacker in self.attackers(not piece_color, piece)
                   if self.is_stronger_piece_attacked_by(attacker, piece))
