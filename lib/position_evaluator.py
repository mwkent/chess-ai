...
# Knight
KNIGHT_RANK_EVAL = [[0,0], [5,0], [10,0], [15,0], [30,0], [50,0], [40,0], [20,0]]
...
SQUARES_TO_WHITE_KING_SAFETY = dict.fromkeys([chess.A1, chess.B1, chess.G1, chess.H1], [30, 0])
SQUARES_TO_WHITE_KING_SAFETY.update(dict.fromkeys([chess.A2, chess.B2, chess.C1, chess.G2, chess.H2], [20, 0]))
SQUARES_TO_BLACK_KING_SAFETY = dict.fromkeys([chess.A8, chess.B8, chess.G8, chess.H8], [30, 0])
SQUARES_TO_BLACK_KING_SAFETY.update(dict.fromkeys([chess.A7, chess.B7, chess.C8, chess.G7, chess.H7], [20, 0]))
...
def get_attacking_bonus(board, color, piece, squares_to_attacking_bonus=SQUARES_TO_ATTACKING_BONUS, \
	...
	for attacked_square in board.attacks(piece):
		bonus += get_eval(board, color, squares_to_attacking_bonus.get(attacked_square))
		bonus += get_eval(board, color, ranks_to_attacking_bonus.get(chess_util.get_adjusted_rank(attacked_square, color)))
	return bonus
...
def get_pawn_value(board, turn, free_to_take):
	...
	for pawn in pawns:
		if pawn == free_to_take:
			evaluation += PIECE_TYPES_TO_VALUES[chess.PAWN] * FREE_TO_TAKE_MODIFIER_PENALTY
		else:
			...
			if is_passed_pawn(board, pawn):
				...
				evaluation += get_eval(board, turn, PAWN_RANK_BONUS) * get_adjusted_pawn_rank(pawn, turn) * 2
				...
				evaluation += get_eval(board, turn, PAWN_RANK_BONUS) * get_adjusted_pawn_rank(pawn, turn)
...
def get_kick_knight_penalty(board, knight):
	...
	kicker_squares = [chess_util.add_rank_and_file(knight, 2*color_modifier, 1), chess_util.add_rank_and_file(knight, 2*color_modifier, -1)]
	if chess.square_rank(knight) == two_push_knight_rank:
		kicker_squares += [chess_util.add_rank_and_file(knight, 3*color_modifier, 1), chess_util.add_rank_and_file(knight, 3*color_modifier, -1)]
	...
	if not chess_util.can_piece_be_captured(board, knight) and not board.is_pinned(knight_color, knight):
		...
		for attacked_piece in board.attacks(knight):
			if board.piece_type_at(attacked_piece) in [chess.ROOK, chess.QUEEN, chess.KING] and board.color_at(attacked_piece) != knight_color:
				value = all_piece_types_to_values[board.piece_type_at(attacked_piece)]
...
def get_knight_value(board, turn, free_to_take, free_to_trade, free_to_trade_value):
	...
	for knight in knights:
		if knight == free_to_take:
			evaluation += PIECE_TYPES_TO_VALUES[chess.KNIGHT] * FREE_TO_TAKE_MODIFIER_PENALTY
			...
			if not board.is_pinned(turn, knight):
				...
				evaluation += get_eval(board, turn, get_attacking_bonus(board, turn, knight))
			...
			evaluation += get_eval(board, turn, get_developed_eval(knight, turn))
			evaluation += get_eval(board, turn, get_kick_knight_penalty(board, knight))
...
def is_piece_on_bishop_color(board, piece):
	...
	return chess_util.on_light_squares(bishops) and not chess_util.on_dark_squares(bishops) and chess_util.is_piece_on_light_square(piece) or \
		chess_util.on_dark_squares(bishops) and not chess_util.on_light_squares(bishops) and chess_util.is_piece_on_dark_square(piece)
...
def get_long_diagonal_bonus(bishop):
	long_diagonals = chess.SquareSet.ray(chess.A1, chess.B2).union(chess.SquareSet.ray(chess.A8, chess.B7))
...
def get_undeveloped_bishop_blocked_penalty(board, bishop):
	...
	if chess_util.get_adjusted_rank(bishop, color) == 0:
		...
		blocking_squares = [chess_util.add_rank_and_file(bishop, rank_adjustment, 1), chess_util.add_rank_and_file(bishop, rank_adjustment, -1)]
...
def get_bishop_value(board, turn, free_to_take, free_to_trade, free_to_trade_value):
	...
	for bishop in bishops:
		if bishop == free_to_take:
			evaluation += PIECE_TYPES_TO_VALUES[chess.BISHOP] * FREE_TO_TAKE_MODIFIER_PENALTY
			...
			if not chess_util.is_bishop_pinned(board, bishop, turn):
				...
				evaluation += get_eval(board, turn, get_attacking_bonus(board, turn, bishop))
				evaluation += get_eval(board, turn, get_long_diagonal_bonus(bishop))
			evaluation += get_defended_bonus(board, bishop)
			evaluation += get_bishop_battery_bonus(board, bishop, turn)
			evaluation += get_eval(board, turn, get_developed_eval(bishop, turn))
...
def get_rook_value(board, turn, free_to_take, free_to_trade, free_to_trade_value):
	...
	for rook in rooks:
		if rook == free_to_take:
			...
			evaluation += get_eval(board, turn, get_rook_on_open_file_bonus(board, rook))
...
def activate_king(board, color):
	...
	pawns = board.pieces(chess.PAWN, chess.WHITE).union(board.pieces(chess.PAWN, chess.BLACK))
	distance_to_pawn_bonus = get_eval(board, color, KING_DISTANCE_TO_PAWN_BONUS)
	value = 0
	for pawn in pawns:
...
def get_king_safety(board, color):
	...
	attacking_adjacent_value = -percentage_attacked_adjacent * get_eval(board, color, ATTACKING_ADJACENT_EVAL)
...
def get_pawn_wall_value(board, color):
	...
	close_wall_squares = [square_in_front, chess_util.add_file(square_in_front, -1), chess_util.add_file(square_in_front, 1)]
...
def get_defended_bonus(board, piece):
	color = board.color_at(piece)
	defenders = [defender for defender in board.attackers(color, piece) if not board.is_pinned(color, defender)]
	if len(defenders) >= 2:
...
def evaluate_position(board, turn, check_tactics=True, extend=True):
	...
	evaluation = 0
	evaluation += get_evaluation_for_both_sides(get_pawn_value, board, turn, free_to_take)
	evaluation += get_evaluation_for_both_sides(get_knight_value, board, turn, free_to_take, free_to_trade, free_to_trade_value)
	evaluation += get_evaluation_for_both_sides(get_bishop_value, board, turn, free_to_take, free_to_trade, free_to_trade_value)
	evaluation += get_evaluation_for_both_sides(get_rook_value, board, turn, free_to_take, free_to_trade, free_to_trade_value)
	evaluation += get_evaluation_for_both_sides(get_queen_value, board, turn, free_to_take, free_to_trade, free_to_trade_value)
	evaluation += get_evaluation_for_both_sides(get_king_value, board, turn, free_to_take, free_to_trade, free_to_trade_value)
	evaluation += get_evaluation_for_both_sides(attacking_stronger_pieces, board, turn, free_to_take, free_to_trade, free_to_trade_value)
	#print(board)
