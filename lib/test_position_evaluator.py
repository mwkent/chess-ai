...
class TestPositionEvaluator(unittest.TestCase):
	...
	def test_disconnected_rooks(self):
		board = Board("r3k2r/pbppqppp/1p3n2/2b5/4P3/2N3P1/PPP2PBP/R1BQ1RK1 b kq - 6 10")
	...
	def test_get_rook_on_open_file_bonus(self):
		board = Board("5rk1/5ppp/4p3/8/8/8/5PPP/3RR1K1 w - - 0 1")
		rook = chess.D1
	...
	def test_get_rook_too_aggressive_penalty(self):
		board = Board("3qkb1r/pppp1ppp/2n2n2/4p3/R3P2r/4RN2/PPPP1PPP/2BQ2K1 w k - 0 1")
		rook = chess.A4
	...
	def test_get_rook_aligned_with_bishop_penalty(self):
		board = Board("5rk1/p1pqpp1p/2n2np1/3p1b2/1r6/2NPP1PP/P1PB1PBR/R2Q1K2 w - - 1 15")
		rook = chess.B4
	...
	def test_is_passed_pawn(self):
		board = Board("4k3/p7/3pP3/3P4/1P6/8/7P/4K3 w - - 0 1")
		pawn = chess.A7
	...
	def test_one_passed_pawn(self):
		board = Board("4k3/p6p/8/8/8/8/4P3/2K5 w - - 0 1")
	...
	def test_get_rook_behind_pawn_bonus(self):
		board = Board("r3k3/8/5rp1/P7/8/8/R7/R3K3 w - - 0 1")
		pawn = chess.A5
	...
	def test_free_knight(self):
		...
		print()
		board = Board("r1bqkb1r/ppp2ppp/5p2/8/3p4/2N5/PPP1PPPP/R1BQKB1R w KQkq - 0 7")
		evaluation = position_evaluator.evaluate_position(board, chess.BLACK)
		print("took pawn evaluation =", evaluation)
		print()
		board = Board("r1bqkb1r/ppp1pppp/5P2/8/8/2p5/PPP1PPPP/R1BQKB1R w KQkq - 0 7")
		evaluation = position_evaluator.evaluate_position(board, chess.BLACK)
		print("took knight evaluation =", evaluation)

	def test_get_queen_aligned_value(self):
		board = Board("r1bqkb1r/pppp1ppp/2n2n2/1B2p1B1/4P3/5N2/PPP2PPP/RN1RK2Q w Qkq - 0 1")
	...
	def test_get_undeveloped_bishop_blocked_penalty(self):
		board = Board("rnbqkbnr/ppp1pp1p/6p1/3p4/P1P1P3/1B6/1P1PQPPP/BN2KBNR w Kkq - 0 1")
		bishop = chess.A1
	...
	def test_can_knight_catch_pawn(self):
		board = Board("8/1p4K1/1P6/5P2/1k6/8/8/1n6 w - - 0 1")
		pawn = chess.F5
		knight = chess.B1
		self.assertFalse(position_evaluator.can_knight_catch_pawn(board, knight, pawn))

		board = Board("8/1p4K1/1P6/5P2/1k6/8/2n5/8 w - - 0 1")
		pawn = chess.F5
		knight = chess.C2
	...
	def test_is_pawn_promoting(self):
		...
		board = Board("8/8/1ppp4/3k2P1/8/8/8/5K2 w - - 0 1")
		pawn = chess.G5
		color = chess.WHITE
		...
		board = Board("8/1p2k1K1/1P6/5P2/8/8/8/8 w - - 0 1")
		pawn = chess.F5
		color = chess.WHITE
		self.assertTrue(position_evaluator.is_pawn_promoting(board, pawn, color))
	...
	def test_turn_will_not_claim_draw_when_winning(self):
		turn = chess.WHITE
		board = Board("2q4k/2n1Q1b1/2P4p/4P3/1PR5/8/5PPP/6K1 b - - 4 57")
		board.push_uci("g7f8")
		board.push_uci("e7f6")
		board.push_uci("f8g7")
		board.push_uci("f6e7")
	...
	def test_get_knight_fork_value(self):
		board = Board("4q3/8/5N2/8/2k3r1/4N3/4N3/2K3Q1 w - - 0 1")
		knight = chess.F6
	...
	def test_activate_king(self):
		board = Board("8/8/3k4/8/2PPP3/3K4/8/8 w - - 0 1")
		num_pawns = 3
