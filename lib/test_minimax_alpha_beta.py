...
class TestMinimaxAlphaBeta(unittest.TestCase):
	...
	def test_knight_blunder2(self):
		board = chess.Board("r3r1k1/2p2p1p/1pn1q1p1/3pp2n/8/2P1PPPP/pRQP2BK/B4R2 b - - 1 27")
		move = minimax_alpha_beta.pick_move(board, 1)
		self.assertNotEqual(move, chess.Move.from_uci("h5g3"))

	# rook and pawn
	def test_mate_with_rook(self):
		board = chess.Board("8/4k3/1R6/5P2/5K2/8/8/8 b - - 51 130")
		num_moves = 0
	...
	def test_pinned_defender(self):
		board = chess.Board("b6k/4rnq1/p2ppN1p/3P4/1P3P1Q/2P3P1/7P/R3R2K w - - 0 48")
		move = minimax_alpha_beta.pick_move(board, 1)
	...
	def test_bishop_blunder(self):
		board = chess.Board("r1bq1rk1/pp1n1nb1/2p1Q2p/8/P2P4/2N1BN2/1PP2PPP/R4RK1 w - - 1 18")
		move = minimax_alpha_beta.pick_move(board, 1)
		self.assertNotEqual(move, chess.Move.from_uci("e3h6"))

	# Don't go for draw if winning
	def test_draw_by_rep(self):
		board = chess.Board("r5k1/2p2p2/1p1q2p1/3p3p/4p3/3rP1PN/n1N2P1K/R3Q3 b - - 4 43")
	...
	def test_fork_threat(self):
		board = chess.Board("r3kbnr/pN1bq1p1/2p2p2/3p3p/P2Pn3/5N2/1PP1PPPP/R1BQKB1R w KQkq - 1 15")
		move = minimax_alpha_beta.pick_move(board, 2)
		self.assertIn(move, [chess.Move.from_uci("b7c5"), chess.Move.from_uci("c2c3"), chess.Move.from_uci("c1d2")])

	def test_free_bishop(self):
		board = chess.Board("3r2k1/pp5p/2n1p1p1/2p2P2/P7/2b2N2/1PP2PPP/R5K1 w - - 0 16")
		move = minimax_alpha_beta.pick_move(board, 1)
		self.assertEqual(move, chess.Move.from_uci("b2c3"))

	def test_check_to_win_queen(self):
		board = chess.Board("r4rk1/pp2bppp/4b3/2p5/2q1NR2/P3P3/1BQP2PP/5RK1 w - - 0 21")
		move = minimax_alpha_beta.pick_move(board, 1)
		self.assertEqual(move, chess.Move.from_uci("e4f6"))

	def test_recapture_bishop(self):
		board = chess.Board("r1bq1rk1/pp2bppp/2npp3/3BP3/3P4/1Q3N2/PP3PPP/RNB2RK1 b - - 0 10")
		move = minimax_alpha_beta.pick_move(board, 2)
	...
	def test_counterattack(self):
		board = chess.Board("3r1q1k/1p2r1pb/pnp1B2p/8/P1QP2P1/1PN1PP2/2PR1R1P/6K1 w - - 7 36")
		move = minimax_alpha_beta.pick_move(board, 1)
