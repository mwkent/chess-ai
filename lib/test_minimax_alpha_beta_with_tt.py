...
class TestMinimaxAlphaBetaWithSort(unittest.TestCase):
    ...
    def test_mate_in_1(self):
        ...
        move = minimax_alpha_beta.pick_move_ids_with_tt(board, 3)
        print(move)
    ...
    def test_avoid_mate(self):
    	...
    	board.pop()
    	move = minimax_alpha_beta.pick_move_ids_with_tt(board, 2)
    	print(move)
    ...
    def test_dont_sack_queen(self):
        board = Board("1r1q1nkb/1bp1rp1p/pp1pp1pP/8/3PP3/N2B1NPQ/PPP1KP2/1R5R w - - 2 20")
        move = minimax_alpha_beta.pick_move_ids_with_tt(board, 1)
        print(move)
        self.assertNotEqual(move, chess.Move.from_uci("h3e6"))

    def test_push_pawn(self):
        board = Board("8/8/8/K3k3/1pPpB3/3P4/8/8 b - - 0 1")
        move = minimax_alpha_beta.pick_move_ids_with_tt(board, 3)
        print(move)
        self.assertEqual(move, chess.Move.from_uci("b4b3"))

    def test_pawn_fork(self):
        board = Board("8/1b6/1p1ppk2/pP5p/P2NPK2/6P1/7P/8 b - - 0 1")
        move = minimax_alpha_beta.pick_move_ids_with_tt(board, 3)
        print(move)
    ...
    def test_checks_then_skewer(self):
        board = Board("2r4k/1b4p1/1p4Bp/8/5R2/7P/P4P2/6K1 b - - 0 1")
        move = minimax_alpha_beta.pick_move_ids_with_tt(board, 6)
        print(move)
